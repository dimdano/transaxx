...
def empty_flops_counter_hook(module, input, output):
    module.__flops__ += 0
    # add flops number in adapt layer parameter
...
def linear_flops_counter_hook(module, input, output):
    ...
    overall_flops = int(np.prod(input.shape) * output_last_dim + bias_flops)
    module.__flops__ += overall_flops
    
    # add flops number in adapt layer parameter
    if hasattr(module, 'flops'):
        module.flops = overall_flops
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    # add flops number in adapt layer parameter
    if hasattr(conv_module, 'flops'):
        conv_module.flops = int(overall_flops)
