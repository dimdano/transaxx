...
template <typename T>
__host__ __device__ void AdaptIm2Col2D(
    const int64_t index, const T *data_im_ptr,
  ...
  // Location in the flattened image adaptes (i.e. which column in a row of the
  // column matrix)
    ...
    for (int64_t x_kern = 0; x_kern < kernel_w; x_kern++) {
      ...
      data_col_ptr[index_col] =
          (x >= 0 && y >= 0 && x < width_im && y < height_im)
              ? data_im_ptr[c_im * height_im * width_im +
                            y * width_im + x]
              : T(0);
...
template <typename T>
__host__ __device__ void AdaptCol2Im2D(
  ...
  const int64_t p_im = (index / (width * height));
    ...
    for (int64_t x_out = x_out_start; x_out < x_out_end; x_out += 1) {
      ...
      if (y_k % dilation_h == 0 && x_k % dilation_w == 0) {
        ...
        int64_t data_col_index =
            c_im * kernel_h * kernel_w * output_height *
                output_width +
            y_k * kernel_w * output_height * output_width +
            x_k * output_height * output_width +
